...
async function connectToWhatsApp() {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        // Check for pending actions with numerical responses
        if (isBotReady() && pendingActions[sender] && !body.startsWith(prefix) && /^[1-9]\d*$/.test(body.trim())) {
            const action = pendingActions[sender];
            const numResponse = body.trim();
            
            // Media Downloader selection
            if (action.type === 'downloader' && ['1', '2'].includes(numResponse)) {
                delete pendingActions[sender];
                
                if (numResponse === '1') { // TikTok option selected
                ...
                else if (numResponse === '2') { // Instagram option selected
                    await sock.sendMessage(from, { 
            ...
            else if (action.type === 'tiktok_option' && ['1', '2'].includes(numResponse)) {
            ...
            // Image style selection
            else if (action.type === 'image_style' && ['1', '2', '3', '4', '5', '6'].includes(numResponse)) {
                ...
                const selectedStyle = styleMap[numResponse];
        ...
        try {
            switch(command) {
                ...
                case 'get':
                    ...
                    else if (getType === 'send') {
    // Check if sender is the bot owner
    if (sender !== ownerNumber + '@s.whatsapp.net') {
        await sock.sendMessage(from, { text: styles.error + ' _Only the bot owner can use this command_' }, { quoted: msg });
        break;
    }
    ...
    if (!msg.message.extendedTextMessage || !msg.message.extendedTextMessage.contextInfo) {
        await sock.sendMessage(from, { 
            text: styles.warn + ' _Reply to a view-once message with .get send command_'
        }, { quoted: msg });
        break;
    }
    ...
    try {
        ...
        // Check if the message is view-once
        if (!quotedMsg.viewOnceMessage && !quotedMsg.quotedMessage?.viewOnceMessage) {
            await sock.sendMessage(from, { 
                text: styles.error + ' _The replied message is not a view-once media_'
            }, { quoted: msg });
            break;
        }
        
        await sock.sendMessage(from, { text: styles.processing + '\n_Retrieving view-once media..._' }, { quoted: msg });
        
        // Get view-once message content
        const viewOnceMsg = quotedMsg.viewOnceMessage || quotedMsg.quotedMessage.viewOnceMessage;
        ...
        if (viewOnceMsg.message.imageMessage) {
            ...
            const mediaMsg = {
                ...
                message: {
                    imageMessage: viewOnceMsg.message.imageMessage
                }
            };
            ...
            await sock.sendMessage(from, { 
                image: buffer,
                caption: styles.title('ᴠɪᴇᴡ-ᴏɴᴄᴇ ɪᴍᴀɢᴇ ʀᴇᴄᴏᴠᴇʀᴇᴅ') + '\n\n' +
                         styles.bullet(`Caption: ${viewOnceMsg.message.imageMessage.caption || 'No caption'}`) + '\n\n' +
                         styles.success + ' _Successfully recovered view-once image_ ✓'
            }, { quoted: msg });
            
            logActivity('VIEW-ONCE', `View-once image recovered by owner`);
        } 
            ...
            const mediaMsg = {
                ...
                message: {
                    videoMessage: viewOnceMsg.message.videoMessage
                }
            };
            ...
            await sock.sendMessage(from, { 
                video: buffer,
                caption: styles.title('ᴠɪᴇᴡ-ᴏɴᴄᴇ ᴠɪᴅᴇᴏ ʀᴇᴄᴏᴠᴇʀᴇᴅ') + '\n\n' +
                         styles.bullet(`Caption: ${viewOnceMsg.message.videoMessage.caption || 'No caption'}`) + '\n\n' +
                         styles.success + ' _Successfully recovered view-once video_ ✓',
                mimetype: 'video/mp4'
            }, { quoted: msg });
            
            logActivity('VIEW-ONCE', `View-once video recovered by owner`);
        }
        else {
            await sock.sendMessage(from, { 
                text: styles.error + ' _Unsupported view-once media type_\n\n_Only images and videos are supported_'
            }, { quoted: msg });
        }
    } catch (error) {
        console.error('View-once recovery error:', error);
        logActivity('ERROR', `View-once recovery error: ${error.message}`);
        await sock.sendMessage(from, { 
            text: styles.error + ' _Failed to recover view-once media_\n\n_The message might be expired or inaccessible_ ⚠️'
        }, { quoted: msg });
    }
}
else {
    await sock.sendMessage(from, { text: styles.warn + ' _Available options: .get foto, .get info, or .get send_' }, { quoted: msg });
}
                ...
                // Image to Anime Converter (FIXED VERSION)
                case 'toanime':
                    ...
                    if (hasImage) {
                        ...
                        try {
                            ...
                            // Convert image to base64 for direct API processing
                            ...
                            // Create a unique filename for potential temporary storage
                            ...
                            const animeResponse = await axios.post('https://api.ryzendesu.vip/api/ai/direct-toanime', formData, {
                                headers: formData.getHeaders(),
                                responseType: 'arraybuffer'
                            });
                            
                            // Save the response image temporarily
                            fs.writeFileSync(tempFilename, Buffer.from(animeResponse.data));
                            
                            // Send processed image
                            await sock.sendMessage(from, { 
                                image: { url: tempFilename },
